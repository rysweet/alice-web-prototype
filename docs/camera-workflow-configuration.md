...
| `--project <file.a3p>` | no | Starter project; launching a project resets camera workflow state |
...
Camera workflow state belongs to the active server session.

| Server action | Camera workflow effect |
| --- | --- |
| `createServer()` | Creates a new isolated default camera workflow state |
| `POST /api/launch` | Resets camera workflow state for the launched project |
| `POST /api/project/new` | Replaces the active project and resets camera workflow state |
| `POST /api/project/save` | Leaves camera workflow state unchanged |
| Server process restart | Clears in-memory markers and restores default state |

There is no shared singleton camera state across server instances.

## Marker storage

Markers are stored in memory with the active server session. They are intended
for authoring, tests, and local workflow parity, not cross-process persistence.
...
Deleting a marker removes it from the active session only. Saving or restoring a
project does not serialize markers into the `.a3p` file.
