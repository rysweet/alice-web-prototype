...
This tutorial describes the imported model and texture workflow. The browser controls, REST routes, project fields, and selectors are runnable end to end.
...
3. A saved `.a3p` archive that contains the matching resource bytes under
   `resources/models/` and `resources/textures/`.

## Prerequisites

Install dependencies and start Alice:
...
Use `.glb` for models with binary buffers or embedded images. Use `.gltf` when
the file is self-contained or its referenced data is available through the
project archive.
...
The project records the asset like this:
...
These asset import routes need a 25 MiB JSON body limit because base64
uploads can exceed the current general API JSON limit.
...
- Texture binding is intentionally narrow: one `"surface"` target per shape.
