...
function buildBootstrapPrelude(): string {
  return [
    ...
    'const resources = JSON.parse(readText("alice-export-resources") || "{}");',
...
function buildObjectCreation(): string {
  return [
    ...
    '    const resourcePathForId = (id) => { const text = String(id || ""); if (text.startsWith("project/models/")) return "resources/models/" + text.slice("project/models/".length); if (text.startsWith("project/textures/")) return "resources/textures/" + text.slice("project/textures/".length); return ""; };',
    '    let color = 0x8888cc;',
    ...
    '    const materialOptions = { color, roughness: 0.72, metalness: 0.08 };',
    '    const surfaceBinding = Array.isArray(object.materialBindings) ? object.materialBindings.find((binding) => binding && binding.target === "surface") : null;',
    '    const textureResourceId = surfaceBinding?.textureResourceId || "";',
    '    const textureUrl = resources[resourcePathForId(textureResourceId)] || "";',
    '    if (textureUrl) { materialOptions.color = 0xffffff; materialOptions.map = new THREE.TextureLoader().load(textureUrl, () => playerState.render()); materialOptions.map.colorSpace = THREE.SRGBColorSpace; }',
    '    const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial(materialOptions));',
    '    const modelResourceId = object.modelResourceId || "";',
    '    const modelResourceUrl = resources[resourcePathForId(modelResourceId)] || "";',
    '    mesh.userData.aliceResources = { modelResourceId, modelResourceAvailable: Boolean(modelResourceUrl), textureResourceId, textureResourceAvailable: Boolean(textureUrl) };',
