...
export class ParallelAnimation extends BaseAnimation {
  ...
  override update(deltaMs: number): AnimationClipState { this.beginIfNeeded(); const safeDelta = sanitizeDelta(deltaMs); let allComplete = true; for (const child of this.#children) { if (!child.isComplete) { child.update(safeDelta); if (!child.isComplete) allComplete = false; } } this.elapsedMsInternal = Math.min(this.elapsedMsInternal + safeDelta, this.durationMs); this.completeInternal = allComplete; this.notifyUpdated(); if (this.completeInternal) this.finishIfNeeded(); return animationStateOf(this); }
